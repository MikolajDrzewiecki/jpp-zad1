...
module Blockchain where
    ...
    mineBlock miner parentHash txs = 
      let header = BlockHeader { 
        ...
        nonce = 0
          ...
          findNonce (BlockHeader p c t n)
            ...
            | otherwise = findNonce (BlockHeader p c t (n + 1))
    ...
    validChain bs = verifyChain bs /= Nothing 
    
    verifyChain :: [Block] -> Maybe Hash
    verifyChain [] = Just 0
    verifyChain [b] = verifyBlock b 0
    verifyChain (b:bs) = let parentHash = verifyChain bs in
      case parentHash of
        Nothing -> Nothing
        Just h -> verifyBlock b h
